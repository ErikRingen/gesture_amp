...
plot_bs_ordpred <- function(m_rank_bws, m_bond_bws, d_gesture){
...
bond_seq <- seq(from = mean_bond - 2*sd_bond, to = mean_bond + sd_bond*2, length.out = 30) - mean_bond
rank_seq <- seq(from = mean_rank - 2*sd_rank, to = mean_rank + sd_rank*2, length.out = 30)
...
pred_rank_B_summary <- pred_rank_B_long %>% 
  group_by(amp, mean_rank_diff, modality, species) %>% 
...
pred_rank_B_summary$rank_z_score <- (pred_rank_B_summary$mean_rank_diff - mean_rank) / sd_rank
...
p_rank <- ggplot(pred_rank_B_summary, aes(x = rank_z_score, y = med, color = amp, fill = amp)) +
  ...
  xlab("Rank Diff. (between-subjects z-score)") +
  ylab("Pr(Amplitude)") +
  labs(fill = "Amplitude") +
...
pred_bond_B_summary <- pred_bond_B_long %>% 
  group_by(amp, mean_bond_strength, modality, species) %>% 
  summarise(med = median(est),
            lower = PI(est, prob = 0.95)[1],
            upper = PI(est, prob = 0.95)[2])
...
pred_bond_B_summary$bond_z_score <- (pred_bond_B_summary$mean_bond_strength - mean_bond) / sd_bond
...
p_bond <- ggplot(pred_bond_B_summary, aes(x = bond_z_score, y = med, color = amp, fill = amp)) +
  ...
  xlab("Bond Strength (between-subjects z-score)") +
  ylab("Pr(Amplitude)") +
  labs(fill = "Amplitude") +
  labs(color = "Amplitude") +
  ggtitle("") +
...
ggsave("figures/plot_bs_ordpred.pdf", plot = p_both, width = 8.5, height = 11)
